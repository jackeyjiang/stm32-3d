...
//= step data
    //== 在一个移动块内
    static  boolean stepBlockFlag[VIRTUAL_AXIS];
    ...
    static inline void disableXStepper()
    {
        ...
        digitalWrite(X2_ENABLE_PIN,!X_ENABLE_ON);
#endif
    }
    ...
    {
#if (Y_ENABLE_PIN > -1)
        digitalWrite(Y_ENABLE_PIN,!Y_ENABLE_ON);
#endif
#if FEATURE_TWO_YSTEPPER && (Y2_ENABLE_PIN > -1)
        digitalWrite(Y2_ENABLE_PIN,!Y_ENABLE_ON);
#endif
    }
    ...
    {
#if (Z_ENABLE_PIN > -1)
        digitalWrite(Z_ENABLE_PIN,!Z_ENABLE_ON);
#endif
#if FEATURE_TWO_ZSTEPPER && (Z2_ENABLE_PIN > -1)
        digitalWrite(Z2_ENABLE_PIN,!Z_ENABLE_ON);
#endif
    }
    /** \brief Enable stepper motor for x direction. */
    static inline void  enableXStepper()    {digitalWrite(X_ENABLE_PIN, X_ENABLE_ON); }
    /** \brief Enable stepper motor for y direction. */
    static inline void  enableYStepper()    {digitalWrite(Y_ENABLE_PIN, Y_ENABLE_ON); }
    /** \brief Enable stepper motor for z direction. */
    static inline void  enableZStepper()    {digitalWrite(Z_ENABLE_PIN, Z_ENABLE_ON);}
    ...
    static inline void setXDirection(bool positive)
    {
        if(positive)
        {
            ...
            digitalWrite(X2_DIR_PIN,!INVERT_X_DIR);
#endif
        }
        else
        {
    ...
    static inline void setYDirection(bool positive)
    {
        if(positive)
        {
            ...
            digitalWrite(Y2_DIR_PIN,!INVERT_Y_DIR);
#endif
        }
        else
        {
    ...
    static inline void setZDirection(bool positive)
    {
        if(positive)
        {
            ...
            digitalWrite(Z2_DIR_PIN,!INVERT_Z_DIR);
#endif
        }
        else
        {
